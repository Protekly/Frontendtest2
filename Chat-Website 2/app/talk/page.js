...
const Talk = () => {
  ...
  const [showWaveIcon, setShowWaveIcon] = useState(true);
  const [messages, setMessages] = useState([
    {
      message: "Hey there, great to meet you. I’m Pi, your personal AI.",
      type: 2
    },
    {
      message: "My goal is to be useful, friendly and fun. Ask me for advice, for answers, or let’s talk about whatever’s on your mind.",
      type: 1
    },
    {
  ...
  return (
    <div className="bg-bg_color h-[100vh] w-full flex relative">
      {/* side-bar */}
      <div className="lg:w-[6%] h-full border border-neutral-300 text-center pt-6 px-2 lg:flex hidden flex-col gap-4">
        <a
          href="./discover"
          className="p-2 flex flex-col items-center gap-1 hover:bg-custom_color_200 hover:rounded-xl "
        >
          <svg viewBox="0 0 32 32" fill="rgb(107, 98, 85)" class="h-8 w-8">
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
        ...
        <a
          ...
          className="p-2 flex flex-col items-center gap-1 hover:bg-custom_color_200 hover:rounded-xl"
        >
          <svg width="24" height="24" fill="rgb(107, 98, 85)">
      ...
      <div className="px-6 fixed  flex items-center justify-between lg:w-[94%] w-full mt-8 right-0">
        <button className="lg:invisible flex h-9 w-9 items-center justify-center rounded-full p-1.5 text-primary-700 bg-custom_color_300 hover:bg-custom_color_300 active:bg-neutral-300-tap">
        ...
        {/* Dropdown  */}
        <div className="relative bg-bg_color_100 rounded-full">
          <div className={`bg-bg_color_100 rounded-full flex items-center gap-3 p-2 relative ${activeDrop ? '':  'opacity-75' }`}>
            ...
            {soundOn ? (
              <HiMiniSpeakerWave className="text-2xl" onClick={toggleSound} />
            ) : (
              <HiSpeakerXMark className="text-2xl" onClick={toggleSound} />
            )}
            </button>

            <button className="flex gap-2" onClick={()=> setSoundOn(!soundOn)}>
            {dropdownVisible && (
              <span className="text-xs">{soundOn ? "Voice on" : "Voice off"}</span>
            )}
            </button>
          ...
          {dropdownVisible && (
            <div className="absolute top-10 right-0 bg-bg_color_100 w-full p-2 rounded-md">
              {/* Example data in the dropdown */}
        ...
        <div className="max-w-[650px] w-full m-auto">
          <div>
            <div className="px-8 py-36">
              ...
              <ul className="flex flex-col gap-4 w-full text-xl text-text_color_100">

              {messages.map((item, i)=>{
                return  <li key={i} className={`flex ${item.type === 2 ? "justify-end": "justify-start"}`}>
                <span className={`p-2 rounded-xl ${item.type === 2 ? " bg-[#f5eadb] text-right": "text-left"}`}> {item.message}</span>
                </li>
              })}
      ...
      <div className="text-center lg:w-[94%] w-full flex items-center flex-col fixed bottom-0 right-0">
        <div className=" max-w-[650px] w-full flex items-center relative">
          ...
          />
          <div
            className={`absolute right-7 flex h-9 w-9 items-center cursor-default justify-center rounded-full ${
              isInputFilled ? "bg-underline cursor-pointer " : "bg-bg_color"
            }`}
          >
            <FiArrowUp
              className={`text-${
                isInputFilled ? "white" : ""
              } font-bold bg_color_100`}
            />
          </div>
        </div>
        <div className="py-6 px-5">
          <p className="text-[12px] text-text_color flex gap-1">
            By using Pi, you agree to our
            <Link href="#" className=" underline text-underline">
              Terms
            </Link>
            and
            <Link href="#" className=" underline text-underline">
